...
# 함수 바로 아래 작은 따옴표 3개(""")로 둘러싸인 텍스트는 주석으로 독스트링이라고 함
# 독 스트링은 help()함수나 __doc_ _속성으로 접근할 수 있어 개발자들이 코드를 파악하고 활용하는데 유용함
...
```
### (3) 프롬프트 작성 (클로드 데스크탑에서 실행): 클로드가 해당 도구를 허용할 것인가 물어보면 '한번만 허용' 클릭
...
### (1) 프롬프트 작성하기(스미더리에서 API 설정하고 Desktop Commander 검색후 등록후 아래 프롬프트 실습
...
def create_folder(folder_name: str) -> str:
	...
	"""
         ...
         import os
         folder_path = os.path.join("C:/project/my_mcp/test", folder_name)
...
```
나는 프로그래밍 책을 출판하는 저자야. 매번 아래와 같은 폴더를 만들어야 하는데 이것을 자동화하고 싶어. 아래 폴더 구조와 샘플 코드를 참고삼아 MCP 서버 코드를 작성해 줘.
...
### 3) 엑셀 MCP 서버 가져와서 활용하기
  ...
  - 다음 사이트에서 pyhub.mcptools-windows-v0.9.5.zip 다운로드
       ...
       https://github.com/pyhub-kr/pypub-mcptools/releases
...
- 프롬프트 입력(2)
```
다음 "" 안의 내용을 c:/test 폴더에 있는 report.hwpx 로 저장해줘 
"# 월간보고서
## 프로젝트 현황
* 개발 진행률: 75%
* 주요 이슈: 없음
...
- 스미더리에서 “Sequential Thinking”을 검색하고 @xinzhongyouhai 제작 Sequential Thinking Tools 를 선택하고 클로드 데스크탑을 눌러 설치 병령어를 받고 명령어를 실행
...
- 네이버 개발자 센터(developer.naver.com)에 접속하여 로그인 후 [application] 메뉴을 선택해서 애플리케이션 등록화면으로 이동

- 화면에서 애플리케이션 이름을 작성, 사용 API 3개 선택, 비로그인 오픈 API 서비스 환경에 [WEB 설정]을 선택하고 주소는 https://localhost:8000 입력하고 등록하기 클릭
...
- [Claude Desktop]을 누른 후 네이버의 클라이언트 ID와 시크릿을 입력하고 [connect] 클릭하면 MCP 설치를 위한 명령어를 안내 해줌. 이를 복사해서 명령 프롬프트 또는 터미널에 복사하여 실행
